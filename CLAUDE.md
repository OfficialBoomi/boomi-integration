...
- **Credentials must not reach the command line or the environment.** Auth goes to curl in a config file on stdin (`-K -`), and `load_env` does not export `.env` values. Two consequences: **stdin is reserved** in the wrapper — never `@-`, `-T -`, or piping into it (inline `-d` and `--data-binary @file` are fine) — and do not re-add `set -a` to `load_env`, however idiomatic it looks for sourcing a `.env`.
